...
class cameraGUI(camGUI.cameraGUI):
    ...
    def __init__(self,parent):
        ...
        self.btnQrTxt.set("Scan QR Code")
        self.QRBtn=TK.Button(self.parent,textvariable=self.btnQrTxt,command=self.qrGet)
        readChk=TK.Checkbutton(self.parent,text="Read",variable=self.qrRead)
        streamChk=TK.Checkbutton(self.parent,text="Stream",variable=self.qrStream)
        labelQR=TK.Label(self.parent,textvariable=self.resultQR)
        readChk.grid(row=3,column=0)
        streamChk.grid(row=3,column=0)
        self.QRBtn.grid(row=3,column=3)
        labelQR.grid(row=4,column=4)
        self.scan=False
    def qrGet(self):
        if (self.scan==True):
            self.btnQrTxt.set("Scan QR Code")
    ...
    def qrScanner(self):
        ...
        while self.scan==True:
            self.resultQR.set("Scanning Image...")
            self.update()
            cameraGUI.camCapture(SET.PREVIEW_FILE,SET.QR_SIZE)
            self.resultQR.set("Scanning for QR Code")
            self.update()
            ...
            if len(qrcode)>0:
                ...
                f = open("clothing.txt", "w")
                f.write(qrcode)
                self.scan=False
                found=True
            else:
                self.resultQR.set("No QR Code Found")
        if found:
